...
class EchartEg extends Component {
	componentDidMount() {
		echarts.init(document.getElementById('bar-chart1')).setOption({
			tooltip: {
				trigger: 'axis',
				axisPointer: {
					// 坐标轴指示器，坐标轴触发有效
			...
			series: [
				{
					...
					type: 'bar',
					stack: '成本',
					data: [1.206, 1.205, 1.192, 1.2]
				},
				{
					name: '原料处理',
					type: 'bar',
					stack: '成本',
					data: [0.017, 0.017, 0.016, 0.017]
				},
				{
					name: '铸锭车间',
					type: 'bar',
					stack: '成本',
					data: [0.346, 0.369, 0.325, 0.347]
				},
				{
					name: '开方机加工',
					type: 'bar',
					stack: '成本',
					data: [0.031, 0.039, 0.036, 0.036]
				},
				{
					name: '粘胶切片',
					type: 'bar',
					stack: '成本',
					data: [0.386, 0.387, 0.38, 0.384]
				},
				{
					name: '清洗成本',
					type: 'bar',
					stack: '成本',
					data: [0.051, 0.045, 0.039, 0.044]
				},
				{
					name: '分检成本',
					type: 'bar',
					stack: '成本',
					data: [0.051, 0.039, 0.037, 0.041]
				}
			]
		});
		// 绘制图表
		...
		function generate(n) {
			...
			for (var i = 0; i < n; i++) {
				...
				for (var j = 0; j < 3; j++) {
					data[i][j] =
						(z < 15 ? Math.abs(z) : Math.abs(30 - z)) *
						(i % 30 < 15 ? Math.abs(i % 30) : Math.abs(30 - (i % 30)));
					data[i][0] = i % 30;
					data[i][1] = z;
				}
			}
	...
	render() {
		return (
			<Panel title="Echarts实例">
				<Session title="柱状图">
					<div id="bar-chart" style={{ width: '90%', height: '500px' }} />
				</Session>
				<Session title="堆叠柱状图">
					<div id="bar-chart1" style={{ width: '90%', height: '500px' }} />
				</Session>
				<Session title="饼状图">
					<div id="pie-chart" style={{ width: '90%', height: '500px' }} />
				</Session>

				<Session title="折线图">
					<div id="line-chart" style={{ width: '90%', height: '500px' }} />
				</Session>

				<Session title="3d柱状图">
					<div id="3d-chart" style={{ width: '90%', height: '500px' }} />
				</Session>

				<Session title="3d表面图-蝴蝶">
					<div id="3d-butterfly" style={{ width: '90%', height: '500px' }} />
				</Session>

				<Session title="3d表面图-花朵">
					<div id="3d-flower" style={{ width: '90%', height: '500px' }} />
				</Session>
			</Panel>
		);
	}
